...
* Error recovery, in case there is any broken hanging SSHFS processes, sshfs-box
  will give you option to kill them.
...
* Configuration prompt is displayed when sshfs-box is run for the first time or
  when passing the `--config` flag.
...
* `urls`
  * `Required` field.
  * `Array` of strings of remote locations that sshfs-box will connect to with
    SSHFS.
* `folder`
  * `Required` field.
  * `String` of the local folder where remote locations will be mounted.
  * If the folder does not exist, it will be created automatically.
* With the given example config `username@host:/dir/subdir` will be avaliable in
  `/home/username/remote/username@host:-dir-subdir` after mounting.

## Potential improvements

* Mounting/Unmounting of multiple locations is currently done synchronously,
  maybe we could parallelize if performance is a problem.
* Add support for separator in config urls
  https://github.com/SBoudrias/Inquirer.js/#separator.
